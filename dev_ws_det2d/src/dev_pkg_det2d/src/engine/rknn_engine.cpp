...
nn_error_e RKEngine::LoadModelFile(const char* model_file) {
    ...
    int ret =
        rknn_init(&rknn_ctx_, model, model_len, 0, NULL);  // 初始化rknn context
    if (ret < 0) {
    ...
    ret = rknn_query(rknn_ctx_, RKNN_QUERY_SDK_VERSION, &version,
    ...
    NN_LOG_INFO("model input num: %d, output num: %d", io_num.n_input,
                io_num.n_output);
...
nn_error_e RKEngine::Run(std::vector<tensor_data_s>& inputs,
    ...
    if (inputs.size() != input_num_) {
        NN_LOG_ERROR("inputs num not match! inputs.size()=%ld, input_num_=%d",
                     inputs.size(), input_num_);
        return NN_IO_NUM_NOT_MATCH;
    }
    if (outputs.size() != output_num_) {
        NN_LOG_ERROR(
            "outputs num not match! outputs.size()=%ld, output_num_=%d",
