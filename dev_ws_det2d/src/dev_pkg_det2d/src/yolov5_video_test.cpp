...
// make sure .mp4 to be saved after "ctrl + c"
...
int main(int argc, char** argv) {
    ...
    if (record) {
        ...
        writer = new cv::VideoWriter(
            "result.mp4", cv::VideoWriter::fourcc('m', 'p', '4', 'v'), fps,
    ...
    while (true) {
        ...
        NN_LOG_INFO("Method1 Time: %fms, %fms, %fms", elapsed_1, elapsed_2,
                    duration);
        // 输出帧率：读取视频帧的帧率、模型运行帧率、总帧率
        NN_LOG_INFO("Method1 FPS: %f, %f, %f", 1000.0 / elapsed_1,
                    1000.0 / elapsed_2, fps);
        ...
        if (elapsed_all_2 > 1000) {
            ...
            NN_LOG_INFO("Method2 Time:%fms, FPS:%f, Frame Count:%d",
                        elapsed_all_2, frame_count / (elapsed_all_2 / 1000.0f),
                        frame_count);
            frame_count = 0;
