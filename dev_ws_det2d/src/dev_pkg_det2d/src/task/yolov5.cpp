...
// 构造函数
Yolov5::Yolov5(const std::string& labels_path, float conf_thresh,
               float nms_thresh)
    ...
    if (!yolov5::load_parameters_from_json(labels_path_)) {
...
Yolov5::Yolov5() {
    ...
    labels_path_ = "dev_ws_det2d/src/dev_pkg_det2d/config/labels/coco_yolov5.json";
    conf_thresh_ = 0.25;
    nms_thresh_ = 0.45;
    // Load parameters from JSON file
    if (!yolov5::load_parameters_from_json(labels_path_)) {
...
// 图像预处理
nn_error_e Yolov5::Preprocess(const cv::Mat& img, PreprocessType type,
...
// 后处理
nn_error_e Yolov5::Postprocess(const cv::Mat& img,
                               std::vector<Detection>& objects,
                               PreprocessType type) {
    ...
    yolov5::post_process(
        (int8_t*)output_tensors_[0].data, (int8_t*)output_tensors_[1].data,
        (int8_t*)output_tensors_[2].data, height, width, conf_thresh_,
        nms_thresh_, scale_w, scale_h, out_zps_, out_scales_, &detections);
