...
#include "process/yolov7_postprocess.h"
#include "utils/logging.h"

#include <ctime>

void DetectionGrp2DetectionArray(yolov7::object_detect_result_list& det_grp,
    ...
    for (int i = 0; i < det_grp.count; i++) {
        ...
        det.box = cv::Rect(
            det_grp.results[i].box.left, det_grp.results[i].box.top,
...
// 构造函数
Yolov7::Yolov7(const std::string& labels_path, float conf_thresh,
               float nms_thresh)
    ...
    if (!yolov7::load_parameters_from_json(labels_path_)) {
        NN_LOG_ERROR("Failed to load parameters from JSON file");
    }
}
...
Yolov7::Yolov7() {
    ...
    labels_path_ = "dev_ws_det2d/src/dev_pkg_det2d/config/labels/coco_yolov7.json";
    conf_thresh_ = 0.25;
    nms_thresh_ = 0.45;
    // Load parameters from JSON file
    if (!yolov7::load_parameters_from_json(labels_path_)) {
...
// 加载模型，获取输入输出属性
nn_error_e Yolov7::LoadModel(const char* model_path) {
    ...
    if (input_shapes.size() != 1) {
        NN_LOG_ERROR("yolo input tensor number is not 1, but %ld",
                     input_shapes.size());
    ...
    for (int i = 0; i < output_shapes.size(); i++) {
        ...
        if (output_shapes[i].type != NN_TENSOR_INT8) {
            NN_LOG_ERROR("yolo output tensor type is not int8, but %d",
                         output_shapes[i].type);
            return NN_RKNN_OUTPUT_ATTR_ERROR;
        }
        tensor.attr.type = output_shapes[i].type;
        tensor.attr.index = i;
        tensor.attr.size =
            output_shapes[i].n_elems * nn_tensor_type_to_size(tensor.attr.type);
...
// 图像预处理
nn_error_e Yolov7::Preprocess(const cv::Mat& img, PreprocessType type,
                              cv::Mat& image_preprocess) {
    // 预处理包含：letterbox、归一化、BGR2RGB、NCWH
    // 其中RKNN会做：归一化、NCWH转换，所以这里只需要做letterbox、BGR2RGB

    // letterbox 比例
    float wh_ratio =
        (float)input_tensor_.attr.dims[2] / (float)input_tensor_.attr.dims[1];
    // affine or letterbox
    if (type == AFFINE) {
...
// 运行模型
nn_error_e Yolov7::Run(const cv::Mat& img, std::vector<Detection>& objects) {
    // letterbox or resize后的图像
    cv::Mat image_preprocess;
    // 预处理，支持opencv或rga
    Preprocess(img, AFFINE, image_preprocess);
    // 推理
    Inference();
    // 后处理
    Postprocess(image_preprocess, objects, AFFINE);
    return NN_SUCCESS;
}
...
// 后处理
nn_error_e Yolov7::Postprocess(const cv::Mat& img,
                               std::vector<Detection>& objects,
                               PreprocessType type) {
    int height = input_tensor_.attr.dims[1];
    int width = input_tensor_.attr.dims[2];
    float scale_w = width * 1.f / img.cols;  // 保证为浮点类型
    float scale_h = height * 1.f / img.rows;

    yolov7::object_detect_result_list detections;

    yolov7::post_process(output_tensors_, height, width, conf_thresh_, nms_thresh_,
