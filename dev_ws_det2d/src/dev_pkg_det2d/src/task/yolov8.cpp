
#include "yolov8.h"

#include <memory>

#include "process/preprocess.h"
#include "process/yolov8_postprocess.h"
#include "utils/logging.h"

#include <ctime>

void DetectionGrp2DetectionArray(yolov8::object_detect_result_list& det_grp,
                                 std::vector<Detection>& objects) {
    // 根据当前系统时间生成随机数种子
    std::srand(static_cast<unsigned int>(std::time(nullptr)));

    for (int i = 0; i < det_grp.count; i++) {
        Detection det;
        det.className = det_grp.results[i].name;

        det.box = cv::Rect(
            det_grp.results[i].box.left, det_grp.results[i].box.top,
            det_grp.results[i].box.right - det_grp.results[i].box.left,
            det_grp.results[i].box.bottom - det_grp.results[i].box.top);

        det.confidence = det_grp.results[i].prop;
        det.class_id = 0;
        // generate random cv::Scalar color
        det.color = cv::Scalar(rand() % 255, rand() % 255, rand() % 255);
        objects.push_back(det);
    }
}

// 构造函数
Yolov8::Yolov8(const std::string& labels_path, float conf_thresh,
               float nms_thresh)
    : labels_path_(labels_path),
      conf_thresh_(conf_thresh),
      nms_thresh_(nms_thresh) {
    engine_ = CreateRKNNEngine();
    input_tensor_.data = nullptr;

    // Load parameters from JSON file
    if (!yolov8::load_parameters_from_json(labels_path_)) {
        NN_LOG_ERROR("Failed to load parameters from JSON file");
    }
}

Yolov8::Yolov8() {
    engine_ = CreateRKNNEngine();
    input_tensor_.data = nullptr;

    labels_path_ = "src/dev_pkg_det2d/config/labels/coco_yolov8.json";
    conf_thresh_ = 0.25;
    nms_thresh_ = 0.45;
    // Load parameters from JSON file
    if (!yolov8::load_parameters_from_json(labels_path_)) {
        NN_LOG_ERROR("Failed to load parameters from JSON file");
    }
}
// 析构函数
Yolov8::~Yolov8() {
    if (input_tensor_.data != nullptr) {
        free(input_tensor_.data);
        input_tensor_.data = nullptr;
    }
    for (auto& tensor : output_tensors_) {
        free(tensor.data);
        tensor.data = nullptr;
    }
}

// 加载模型，获取输入输出属性
nn_error_e Yolov8::LoadModel(const char* model_path) {
    auto ret = engine_->LoadModelFile(model_path);
    if (ret != NN_SUCCESS) {
        NN_LOG_ERROR("yolo load model file failed");
        return ret;
    }
    // get input tensor
    auto input_shapes = engine_->GetInputShapes();

    // check number of input and n_dims
    if (input_shapes.size() != 1) {
        NN_LOG_ERROR("yolo input tensor number is not 1, but %ld",
                     input_shapes.size());
        return NN_RKNN_INPUT_ATTR_ERROR;
    }
    nn_tensor_attr_to_cvimg_input_data(input_shapes[0], input_tensor_);
    input_tensor_.data = malloc(input_tensor_.attr.size);

    auto output_shapes = engine_->GetOutputShapes();

    for (int i = 0; i < output_shapes.size(); i++) {
        tensor_data_s tensor;
        tensor.attr.n_elems = output_shapes[i].n_elems;
        tensor.attr.n_dims = output_shapes[i].n_dims;
        for (int j = 0; j < output_shapes[i].n_dims; j++) {
            tensor.attr.dims[j] = output_shapes[i].dims[j];
        }
        // output tensor needs to be float32
        if (output_shapes[i].type != NN_TENSOR_INT8) {
            NN_LOG_ERROR("yolo output tensor type is not int8, but %d",
                         output_shapes[i].type);
            return NN_RKNN_OUTPUT_ATTR_ERROR;
        }
        tensor.attr.type = output_shapes[i].type;
        tensor.attr.index = i;
        tensor.attr.size =
            output_shapes[i].n_elems * nn_tensor_type_to_size(tensor.attr.type);
        tensor.data = malloc(tensor.attr.size);
        output_tensors_.push_back(tensor);
        out_zps_.push_back(output_shapes[i].zp);
        out_scales_.push_back(output_shapes[i].scale);
    }
    return NN_SUCCESS;
}

// 图像预处理
nn_error_e Yolov8::Preprocess(const cv::Mat& img, PreprocessType type,
                              cv::Mat& image_preprocess) {
    // 预处理包含：letterbox、归一化、BGR2RGB、NCWH
    // 其中RKNN会做：归一化、NCWH转换，所以这里只需要做letterbox、BGR2RGB

    // letterbox 比例
    float wh_ratio =
        (float)input_tensor_.attr.dims[2] / (float)input_tensor_.attr.dims[1];
    // affine or letterbox
    if (type == AFFINE) {
        image_preprocess = img;
    } else if (type == LETTERBOX) {
        letterbox_info_ = letterbox_rga(img, image_preprocess, wh_ratio);
    } else {
        NN_LOG_ERROR("unknown preprocess type: %d", type);
        return NN_PREPROCESS_FAIL;
    }

    // BGR2RGB，resize，再放入input_tensor_中
    cvimg2tensor_rga(image_preprocess, input_tensor_.attr.dims[2],
                 input_tensor_.attr.dims[1], input_tensor_);

    return NN_SUCCESS;
}

// 推理
nn_error_e Yolov8::Inference() {
    std::vector<tensor_data_s> inputs;
    // 将input_tensor_放入inputs中
    inputs.push_back(input_tensor_);
    // 运行模型
    engine_->Run(inputs, output_tensors_, false);
    return NN_SUCCESS;
}

// 运行模型
nn_error_e Yolov8::Run(const cv::Mat& img, std::vector<Detection>& objects) {
    // letterbox or resize后的图像
    cv::Mat image_preprocess;
    // 预处理，支持opencv或rga
    Preprocess(img, AFFINE, image_preprocess);
    // 推理
    Inference();
    // 后处理
    Postprocess(image_preprocess, objects, AFFINE);
    return NN_SUCCESS;
}
void letterbox_decode(std::vector<Detection>& objects, bool hor, int pad) {
    for (auto& obj : objects) {
        if (hor) {
            obj.box.x -= pad;
        } else {
            obj.box.y -= pad;
        }
    }
}
// 后处理
nn_error_e Yolov8::Postprocess(const cv::Mat& img,
                               std::vector<Detection>& objects,
                               PreprocessType type) {
    int height = input_tensor_.attr.dims[1];
    int width = input_tensor_.attr.dims[2];
    float scale_w = width * 1.f / img.cols;  // 保证为浮点类型
    float scale_h = height * 1.f / img.rows;

    yolov8::object_detect_result_list detections;

    yolov8::post_process(output_tensors_, height, width, conf_thresh_, nms_thresh_,
                         scale_w, scale_h, out_zps_, out_scales_, &detections);
    
    DetectionGrp2DetectionArray(detections, objects);
    if (type == LETTERBOX) {
        letterbox_decode(objects, letterbox_info_.hor, letterbox_info_.pad);
    }

    return NN_SUCCESS;
}
