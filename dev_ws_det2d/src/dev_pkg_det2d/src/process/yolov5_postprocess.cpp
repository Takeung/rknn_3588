...
int all_class_num_ = 80;
int prop_box_size_ = (5 + all_class_num_);
std::vector<std::string> labels_(obj_class_num_);
int anchor0_[6] = {10, 13, 16, 30, 33, 23};
int anchor1_[6] = {30, 61, 62, 45, 59, 119};
int anchor2_[6] = {116, 90, 156, 198, 373, 326};

// // custom
// static const char* labels_[obj_class_num_] = {
//     "car",     "truck",   "heavy_truck", "van",     "bus",
//     "bicycle", "cyclist", "tricycle",    "trolley", "pedestrain",
//     "cone",    "animal",  "other"};

// // custom
// const int anchor0_[6] = {2, 6, 6, 7, 7, 18};
// const int anchor1_[6] = {14, 11, 21, 16, 29, 23};
// const int anchor2_[6] = {46, 36, 74, 61, 167, 139};
...
bool load_parameters_from_json(const std::string& json_file) {
    ...
    if (json.contains("obj_class_num")) {
        obj_class_num_ = json["obj_class_num"];
        NN_LOG_INFO("Read obj_class_num: %d", obj_class_num_);
    }
    if (json.contains("all_class_num")) {
        all_class_num_ = json["all_class_num"];
        NN_LOG_INFO("Read all_class_num: %d", all_class_num_);
    }
    prop_box_size_ = 5 + all_class_num_;

    if (json.contains("labels")) {
        labels_.reserve(obj_class_num_);
        labels_ = json["labels"].get<std::vector<std::string>>();
    }

    if (json.contains("anchor0")) {
        for (int i = 0; i < 6; ++i) {
            anchor0_[i] = json["anchor0"][i];
        }
        NN_LOG_INFO("Read anchor0: %d %d %d %d %d %d", anchor0_[0], anchor0_[1],
                    anchor0_[2], anchor0_[3], anchor0_[4], anchor0_[5]);
    }
    if (json.contains("anchor1")) {
        for (int i = 0; i < 6; ++i) {
            anchor1_[i] = json["anchor1"][i];
        }
        NN_LOG_INFO("Read anchor1: %d %d %d %d %d %d", anchor1_[0], anchor1_[1],
                    anchor1_[2], anchor1_[3], anchor1_[4], anchor1_[5]);
    }
    if (json.contains("anchor2")) {
        for (int i = 0; i < 6; ++i) {
            anchor2_[i] = json["anchor2"][i];
        }
        NN_LOG_INFO("Read anchor2: %d %d %d %d %d %d", anchor2_[0], anchor2_[1],
                    anchor2_[2], anchor2_[3], anchor2_[4], anchor2_[5]);
    }
...
int loadLabelName(const char* locationFilename, char* label[]) {
    ...
    readLines(locationFilename, label, obj_class_num_);
    return 0;
}
...
static int nms(int validCount, std::vector<float>& outputLocations,
    ...
    for (int i = 0; i < validCount; ++i) {
        ...
        for (int j = i + 1; j < validCount; ++j) {
            ...
            float iou = CalculateOverlap(xmin0, ymin0, xmax0, ymax0, xmin1,
                                         ymin1, xmax1, ymax1);
        ...
        for (int i = 0; i < grid_h; i++) {
            for (int j = 0; j < grid_w; j++) {
                int8_t box_confidence =
                    input[(prop_box_size_ * a + 4) * grid_len + i * grid_w + j];
                if (box_confidence >= thres_i8) {
                    int offset =
                        (prop_box_size_ * a) * grid_len + i * grid_w + j;
                    int8_t* in_ptr = input + offset;
                    float box_x =
                        (deqnt_affine_to_f32(*in_ptr, zp, scale)) * 2.0 - 0.5;
                    float box_y =
                        (deqnt_affine_to_f32(in_ptr[grid_len], zp, scale)) *
                            2.0 -
                        0.5;
                    float box_w =
                        (deqnt_affine_to_f32(in_ptr[2 * grid_len], zp, scale)) *
                        2.0;
                    float box_h =
                        (deqnt_affine_to_f32(in_ptr[3 * grid_len], zp, scale)) *
                        2.0;
...
int post_process(int8_t* input0, int8_t* input1, int8_t* input2, int model_in_h,
    ...
    int validCount0 = 0;
    validCount0 = process(input0, (int*)anchor0_, grid_h0, grid_w0, model_in_h,
    ...
    int validCount1 = 0;
    validCount1 = process(input1, (int*)anchor1_, grid_h1, grid_w1, model_in_h,
    ...
    int validCount2 = 0;
    validCount2 = process(input2, (int*)anchor2_, grid_h2, grid_w2, model_in_h,
    ...
    for (int i = 0; i < validCount; ++i) {
        ...
        const char* label = labels_[id].c_str();
