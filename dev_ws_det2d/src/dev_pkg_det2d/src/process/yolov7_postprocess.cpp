...
int anchors_[3][6] = {{12, 16, 19, 36, 40, 28},
                      {36, 75, 76, 55, 72, 146},
...
bool load_parameters_from_json(const std::string& json_file) {
    ...
    if (json.contains("anchors")) {
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 6; ++j) {
                anchors_[i][j] = json["anchors"][i][j];
            }
        }
        NN_LOG_INFO("Read anchor0: %d %d %d %d %d %d", anchors_[0][0], anchors_[0][1],
                    anchors_[0][2], anchors_[0][3], anchors_[0][4], anchors_[0][5]);
        NN_LOG_INFO("Read anchor1: %d %d %d %d %d %d", anchors_[1][0], anchors_[1][1],
                    anchors_[1][2], anchors_[1][3], anchors_[1][4], anchors_[1][5]);
        NN_LOG_INFO("Read anchor2: %d %d %d %d %d %d", anchors_[2][0], anchors_[2][1],
                    anchors_[2][2], anchors_[2][3], anchors_[2][4], anchors_[2][5]);
    }
...
int post_process(std::vector<tensor_data_s>& output_tensors, int model_in_h,
    ...
    for (int i = 0; i < 3; i++) {
        grid_h = output_tensors[i].attr.dims[2];
        grid_w = output_tensors[i].attr.dims[3];
        stride = model_in_h / grid_h;

        validCount += process_i8(
            (int8_t*)(output_tensors[i].data), (int*)anchors_[i], grid_h, grid_w,
            model_in_h, model_in_w, stride, filterBoxes, objProbs, classId,
    ...
    for (int i = 0; i < validCount; ++i) {
        ...
        //        od_results->results[last_count].box.top,
        //        od_results->results[last_count].box.right,
        //        od_results->results[last_count].box.bottom, label);
        last_count++;
    }
