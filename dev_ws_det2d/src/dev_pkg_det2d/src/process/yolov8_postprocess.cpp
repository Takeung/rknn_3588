...
int prop_box_size_ = (4 + all_class_num_);
...
bool load_parameters_from_json(const std::string& json_file) {
    ...
    prop_box_size_ = 4 + all_class_num_;
...
static int process_i8(int8_t* box_tensor, int32_t box_zp, float box_scale,
    ...
    for (int i = 0; i < grid_h; i++) {
        for (int j = 0; j < grid_w; j++) {
            ...
            for (int c = 0; c < obj_class_num_; c++) {
...
int post_process(std::vector<tensor_data_s>& output_tensors, int model_in_h,
    ...
    // default 3 branch
    int dfl_len = output_tensors[0].attr.dims[1] / 4;
    ...
    for (int i = 0; i < 3; i++) {
        ...
        grid_h = output_tensors[box_idx].attr.dims[2];
        grid_w = output_tensors[box_idx].attr.dims[3];
        stride = model_in_h / grid_h;

        validCount += process_i8(
            (int8_t*)output_tensors[box_idx].data, qnt_zps[box_idx],
            qnt_scales[box_idx], (int8_t*)output_tensors[score_idx].data,
            qnt_zps[score_idx], qnt_scales[score_idx], (int8_t*)score_sum,
